...
export const StoryPlayerOverlay = memo(function StoryPlayerOverlay() {
  ...
  const clipByNodeId = useStoryRuntimeStore((s) => s.clipByNodeId);
  ...
  // 进场预加载全部片段(去重 + resolve),消除切换卡顿。
  const preloadUrls = useMemo(
    () =>
      Array.from(new Set(Object.values(clipByNodeId).filter(Boolean)))
  ...
  return createPortal(
    <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black">
      <button
        onClick={exitPlay}
        className="absolute right-5 top-5 z-10 rounded-full border border-white/15 bg-black/50 p-2 text-white/80 backdrop-blur transition-colors hover:text-white"
